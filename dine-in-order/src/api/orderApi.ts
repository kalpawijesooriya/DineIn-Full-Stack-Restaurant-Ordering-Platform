...
interface ApiOrderItem {
  id: string;
  menuItem: Order['items'][number]['menuItem'];
  quantity: number;
  selectedCustomizations?: Record<string, string[]>;
  specialInstructions?: string;
  itemTotal: number;
...
interface ApiOrder {
  ...
  orderTypeDetails: Order['orderTypeDetails'];
  items?: ApiOrderItem[];
  subtotal: number;
  tax: number;
  total: number;
...
export async function placeOrder(order: Omit<Order, 'id' | 'createdAt' | 'status'>): Promise<Order> {
  ...
  const apiOrder = await apiClient.post<ApiOrder>('/orders', request);
  return mapApiOrderToOrder(apiOrder);
}

export async function getOrders(): Promise<Order[]> {
  const apiOrders = await apiClient.get<ApiOrder[]>('/orders');
  return apiOrders.map(mapApiOrderToOrder);
}

export async function getOrderById(id: string): Promise<Order> {
  const apiOrder = await apiClient.get<ApiOrder>(`/orders/${id}`);
  return mapApiOrderToOrder(apiOrder);
}
