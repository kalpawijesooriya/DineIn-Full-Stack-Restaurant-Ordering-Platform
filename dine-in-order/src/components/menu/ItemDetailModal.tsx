...
export function ItemDetailModal({ itemId, isOpen, onClose }: ItemDetailModalProps) {
  ...
  const itemDetailContent = !loading && !error && item ? (
  ...
  const modalFooter = !loading && !error && item ? (
    <div className="sticky bottom-0 z-10 border-t border-slate-200 bg-white/95 p-4 backdrop-blur">
      <div className="flex items-center gap-3">
        ...
        <Button onClick={handleAddToCart} disabled={!item.isAvailable || missingRequiredGroupIds.size > 0} className="min-w-36">
          Add to Cart
        </Button>
      </div>
    </div>
  ...
  const footerContent = !loading && !error && item ? (
